...
export default function EditWaiverPage() {
    ...
    const loadWaiver = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from("waivers")
          .select("id,gym_id,name,content,created_at,updated_at")
          .eq("id", waiverId)
  ...
  const save = async () => {
    ...
    if (!waiverId) {
      toast.error("Waiver ID is missing.")
      return
    }

    setSaving(true)

    const toastId = toast.loading("Saving waiver...")
    ...
    try {
      ...
      /*
       * Keep existing signup_forms.waiver JSON in sync
      ...
      router.push(`/waivers/${waiverId}`)
      router.refresh()
    } catch (error) {
      toast.dismiss(toastId)

      toast.error(
        error instanceof Error ? error.message : "Unable to save waiver.",
      )
    } finally {
      setSaving(false)
    }
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Edit a reusable agreement that can be attached to any signup form.
            </p>
          </div>
        </div>
            ...
            <aside className="space-y-6">
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-xs leading-5 text-slate-500">
                  After editing this waiver, the updated version will be
                  available to signup forms using it.
                </p>
              </section>
            </aside>
          </div>
